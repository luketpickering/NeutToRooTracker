...
//Input stuff
  ...
  UDBLog("Reading " << nFiles << " input files with "
    << NEntries << " entries.");

//Output stuff
  TFile* outFile = new TFile(NeutToRooTrackerOpts::OutFName.c_str(),"RECREATE");

  if(!outFile->IsOpen()){
    UDBError("Couldn't open output file: "
      << NeutToRooTrackerOpts::OutFName);
    return 8;
  } else {
  ...
  UInt_t TFileUID = 0;
  Double_t EvtWght = 0; Double_t NEntriesInFile = 0;
  for(long entryNum = 0; entryNum < doEntries; ++entryNum){
    ...
    if(NeutTree->GetFile()->GetUniqueID() != TFileUID){
      ...
      if(!flux_numu || !evtrt_numu){ EvtWght = 0; }
      else {
        EvtWght = evtrt_numu->Integral()/flux_numu->Integral();
      }
      NEntriesInFile = NeutTree->GetTree()->GetEntries();
      UDBLog("Opened new file: " << NeutTree->GetFile()->GetName()
        << " on entry " << entryNum << "/" << NEntries << " ("
        << NEntriesInFile << " entries in this file), EvtWght: " << EvtWght);
    }
    ...
    for(int partNum = 0; partNum < vector->Npart(); ++partNum){
      ...
      switch(part.fStatus){
          ...
          if(part.fIsAlive == 1){
            ...
            if(NeutToRooTrackerOpts::SkipNonFS){
              UDBVerbose("Not saving particle status ("
                ...
                << "\") as it was not 'IsAlive' in event: "
                << entryNum);
              continue;
            }
          }
        ...
        case 2:{ // Escaped detector == Good
          if(part.fIsAlive == 1){
            outRooTracker->StdHepStatus[saveInd] = 1;
            UDBWarn("Found NEUT status 2:\"" <<  NEUTStatusCodes[part.fStatus]
              << "\" which was marked as "
            ...
            if(NeutToRooTrackerOpts::SkipNonFS){
              UDBVerbose("Not saving particle status("
                ...
                << outRooTracker->StdHepPdg[saveInd] << ") in event: "
                << entryNum);
              continue;
            }
          }
        ...
        default:{
          UDBWarn("Found unexpected neut fStatus code: " << part.fStatus
            << ":\"" <<  NEUTStatusCodes[part.fStatus] << "\" in event: "
            << entryNum);
